...
class RegistrationForm(FlaskForm):
    ...
    locked = SelectField(
        _l("Locked"), choices=[("No", "No"), ("Yes", "Yes")], default="No"
    )
    team = SelectField(
        _l("Team"),
        choices=[("Compute", "Compute"), ("Core Technology", "Core Technology")],
    )
    role = SelectField(
        _l("Role"), choices=[("Admin", "Admin"), ("Guest", "Guest")], default="Guest"
    )
    connectionStatus = SelectField(
        _l("Connected"), choices=[("No", "No"), ("Yes", "Yes")], default="No"
    )
    active = SelectField(
        _l("Active"), choices=[("No", "No"), ("Yes", "Yes")], default="Yes"
    )
    loginCount = StringField(_l("Login Count"), default=0)
    reset = SelectField(
        _l("Reset"), choices=[("No", "No"), ("Yes", "Yes")], default="No"
    )
