...
(function (root, factory) {
  ...
  return (function () {
    ...
    Loki.prototype.deserializeCollection = function (destructuredSource, options) {
      ...
      if (options.delimited && destructuredSource !=null) {
        workarray = destructuredSource.split(options.delimiter);
        workarray.pop();
      }
      else {
      ...
      len = workarray?workarray.length:0;
    ...
    Loki.prototype.loadJSONObject = function (dbObject, options) {
      ...
      for (i; i < len; i += 1) {
        ...
        if (coll.getData) {
          ...
          clen =coll &&coll.data? coll.data.length:0;
          j = 0;
    ...
    LokiPartitioningAdapter.prototype.loadNextPage = function (callback) {
      ...
      this.adapter.loadDatabase(keyname, function (result) {
       ...
       // console.log(`dlen: ${dlen}`)
        ...
        var isLastPage = dlen>0?(data[dlen - 1] === ""):true;
        if (isLastPage) {
          data.pop();
          dlen = data.length;
